...
int extract_swipe_features_extraction_features(signal_t *signal, matrix_t *output_matrix, void *config_ptr, const float frequency){
  ...
  int ret = numpy::transpose(&input_matrix);
  if (ret != EIDSP_OK) {
    printf("ERR: Failed to transpose matrix (%d)\n", ret);
    EIDSP_ERR(ret);
  }

  output_matrix->buffer[0]= 0;
  output_matrix->buffer[1]= 0;

  for (size_t row = 0; row < input_matrix.rows; row++) {
    matrix_t row_matrix(1, input_matrix.cols, input_matrix.buffer + (row * input_matrix.cols));
    for(size_t col = 0; col < (row_matrix.cols - 1); col++){
      output_matrix->buffer[row] += (row_matrix.buffer[col+1]-row_matrix.buffer[col+1]);
    }
    output_matrix->buffer[row] /= (row_matrix.cols-1);
  }
